#!/usr/bin/env python3
"""Single-variable benchmark sweep for speculative draft depth (n=2, n=3, n=4).

Tests Qwen 3.6 35B A3B MTP + N-gram speculative decoding across different draft depths
using identical prompt, seed, temperature, and context length.
Computes median throughput (tok/s) and acceptance rate across multiple runs.
"""
...
RESULTS_FILE = PROJECT_ROOT / "docs" / "measurements" / "speculative-sweep-n2-n3-n4-2026-09-04.json"
...
def update_runtime_config(n: int) -> None:
    ...
    if CONFIG_FILE.exists():
        try:
            ...
            cfg = {}
    cfg["spec_draft_n_max"] = n
    CONFIG_FILE.write_text(json.dumps(cfg, indent=2))


def restart_service(n: int) -> None:
    print(f"\n🔄 [n={n}] Restarting ARGUS service with spec_draft_n_max={n}...")
    ctl = PROJECT_ROOT / "scripts" / "argus_ctl.sh"
    subprocess.run([str(ctl), "stop"], capture_output=True)

    # Ensure any lingering llama-server or port 8080 is terminated
    for _ in range(20):
    ...
    time.sleep(1.0)
    proc = subprocess.run([str(ctl), "start", "--spec-draft-n-max", str(n)], capture_output=True, text=True)
    ...
    if not healthy:
        raise RuntimeError(f"llama-server failed to answer health on n={n}")

    # Verify /proc cmdline has exact n
    res = subprocess.run(["pgrep", "-f", "llama-server"], capture_output=True, text=True)
    ...
    for pid in pids:
        try:
            ...
            if f"--spec-draft-n-max {n}" in cmdline:
                print(f"   ✅ llama-server healthy (PID {pid}, verified cmdline has --spec-draft-n-max {n})")
    ...
    if not cmdline_verified:
        print(f"   ℹ️ llama-server healthy (n={n})")
...
def main():
    parser = argparse.ArgumentParser(description="Speculative Draft Depth Sweep")
    parser.add_argument("--n-values", nargs="+", type=int, default=[2, 3, 4], help="Draft depths to test")
    ...
    print("=" * 70)
    print("🚀 ARGUS SPECULATIVE DRAFT SWEEP BENCHMARK (n=2, n=3, n=4)")
    print(f"   Runs: {args.runs} | Max Tokens: {args.max_tokens} | Model: Qwen 3.6 35B A3B MTP")
    print("=" * 70)

    for n in args.n_values:
        update_runtime_config(n)
        restart_service(n)

        # Robust Warmup: 128 tokens on target prompt to exercise MoE expert router across layers
        ...
        for r in range(1, args.runs + 1):
            ...
            n_runs.append(run_stat)
            print(f" -> {tps:.2f} tok/s | Acceptance: {acc_rate:.1f}% ({draft_acc}/{draft_n}) | TTFT: {prompt_ms:.0f}ms")
            time.sleep(1.0)

        # Calculate statistics
        tps_list = [x["tokens_per_second"] for x in n_runs]
        acc_list = [x["acceptance_rate_pct"] for x in n_runs]
        ttft_list = [x["prompt_ms"] for x in n_runs]
    ...
    # Summary table
    print("\n" + "=" * 80)
    print("📊 SPECULATIVE DRAFT SWEEP RESULTS (MEDIAN + MIN/MAX SCOREBOARD)")
    print("=" * 80)
    print(f"{'Draft (n)':<10} | {'Median Speed (Min - Max)':<25} | {'Acceptance (Med [Min-Max])':<26} | {'TTFT Prefill':<14} | {'Speedup vs n=2':<12}")
    print("-" * 80)

    base_tps = results["sweep"]["n=2"]["median_tokens_per_second"]
    for n in args.n_values:
        data = results["sweep"][f"n={n}"]
        ...
        speedup = f"{((tps - base_tps) / base_tps) * 100:+.1f}%" if n != 2 else "BASELINE"
        print(f"n = {n:<6} | {tps_range:<25} | {acc_range:<26} | {ttft:<14} | {speedup:>10}")
    print("=" * 80)
