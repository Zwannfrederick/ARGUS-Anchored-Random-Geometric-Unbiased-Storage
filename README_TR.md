# ARGUS: Transformer'lar için Hiyerarşik Sanal Bellek Tabanlı KV Önbellek Çalışma Zamanı
...
ARGUS, Transformer'lar için hiyerarşik sanal bellek tabanlı bir KV önbellek çalışma zamanı sunar. Bu yapı, sabit donanım kısıtları altında bağlam pencerelerini ölçeklemek üzere tasarlanmıştır:
...
```text
                  FP16 Aktif Havuz (Sıcak)
                         ...
                         FP8                          ┐
                          ...
                          ▼                           │  (Yüksek bilgi koruma katmanları)
                         INT8                         │
                          │                           │
                          ▼                           ┘
...
>
> Bu sayede önbellek arşivleme ve yeniden yapılandırma süreçlerinde bilgi kaybı en aza indirilir ve modelin orijinal attention dağılımının bozulması önlenir.
...
| Attention Rekonstrüksiyonu (Kosinüs Benzerliği) | Baseline | %100.00 | Başarılı |
| Aşağı Akış Perplexity Deltası ($\Delta$) | Baseline | < +0.3 | Tahmini |
...
| :--- | :--- | :--- | :--- |
| **2,048 jeton** | 0.0054 | %99.46 | **Yüksek Sadakatli Yeniden Yapılandırma** |
...
*   **Sabit VRAM Bütçesi Altında Kararlı Bağlam Ölçeklemesi:** **sabit VRAM kısıtları altında deneysel olarak tamamlanmış 4.0x'e kadar daha büyük bağlam pencereleri**

### Benchmark Metodolojisi

Maksimum tekrarlanabilirlik ve akademik dürüstlük adına, tüm değerlendirme metrikleri ve kapasite eğrileri aşağıdaki standartlaştırılmış test yapılandırması altında ölçülmüştür:
...
*   **ARGUS Çalışma Zamanı (Runtime):** KV önbelleğini dinamik olarak sıkıştırıp derin arşivleri Host DRAM'e aktararak, **32K bağlam uzunluğunda bile KV önbellek boyutunu 0.8 GB'ın altında tutmayı hedefler**.¹
*   **Sonuç:** 4GB VRAM bütçesi altında çok daha uzun bağlam pencereleriyle kararlı çalışma mümkün hale gelir. ARGUS, kısıtlı değerlendirme kurulumumuzda yüksek zamansal attention lokalitesi yeniden kullanım oranları gözlemlenmiş ve bellek tahsisi kaynaklı OOM arızaları önemli ölçüde azalmıştır.
...
`research` modunda çalıştırıldığında, üretim adımları gerçek zamanlı bir **Sanal Bellek Isı Haritası (Virtual Memory Heatmap)** sunarak VRAM'de duran (`█`) ve CPU'ya taşınmış olan (`▒`) sayfaları gösterir:
...
*   **Sanal Bellek Katmanları (VRAM / CPU DRAM):**
    ...
    *   `▒ CPU Spill`: VRAM baskısı altında Host RAM'e takas edilen sayfalar
...
*   **Kayıplı Sıkıştırma Katmanları:** Agresif soğuk depolama katmanları (1-Bit kuantizasyonu ve Johnson-Lindenstrauss projeksiyonu gibi) kayıplıdır ve bilgi kaybını en aza indirmek üzere tasarlanmış olsa da bazı karmaşık muhakeme zincirlerinde minimal sapmalara yol açabilir. Not: 1-bit arşiv katmanları, yalnızca yeniden kullanılmadan önce geçici rekonstrüksiyon gerektiren derin soğuk pasif sayfalar için tasarlanmıştır; doğrudan attention hesaplamalarında kullanılmazlar.
